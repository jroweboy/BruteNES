...
public:
    ...
    constexpr static u32 NTSC_CLOCK_DIVIDER = 12;
    constexpr static u32 NTSC_PPU_CLOCK = NTSC_MASTER_CLOCK / 4;
    constexpr static u32 NTSC_CPU_CLOCK = NTSC_MASTER_CLOCK / NTSC_CLOCK_DIVIDER;

    void ScheduleInterrupt(s32 cycle_count, InterruptSource source, bool recurring = false);

    s64 CPUCyclesTillInterrupt();

    void AddCPUCycles(s32 cycle_count);
